...
public class UIManagerResult : MonoBehaviour
{
    ...
    public void getJsonContents(string jsonString)
    {
        ...
        UIRes1.transform.Find("PuntosD").gameObject.GetComponent<TMP_Text>().text = "PUNTOS DÉBILES\n" + results.weaknesses;
        UIRes1.transform.Find("Puntuación").gameObject.GetComponent<TMP_Text>().text = "PUNTUACIÓN: " + results.score.ToString();

        UIRes2.transform.Find("Veredicto").gameObject.GetComponent<TMP_Text>().text = "VEREDICTO: " + (results.passed ? "Aprobado" : "No aprobado");
        UIRes2.transform.Find("Justificacion").gameObject.GetComponent<TMP_Text>().text = "JUSTIFICACIÓN: " + results.decision_rationale;
        ...
        UIRes2.transform.Find("STAR").gameObject.GetComponent<TMP_Text>().text = "MÉTODO STAR: " + results.star_method_check;
